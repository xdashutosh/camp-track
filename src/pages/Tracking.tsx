...
import { Users, Loader2, Map as MapIcon, RotateCcw, Clock, MapPin, Navigation } from 'lucide-react';
    ...
    const fetchTrailAndStops = async (workerId: string) => {
        ...
        try {
            const [trailRes, stopsRes] = await Promise.all([
                api.get(`/admin/trails/${workerId}/preset?range=24h&simplify=true`),
                api.get(`/admin/stops/${workerId}/preset?range=24h`)
            ]);
